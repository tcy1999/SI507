...
def map_colorful_agents(agents):  # what argument should go in this function?

    Purple_XCoordinate = [agent.x for agent in agents if agent.group=="Purple"] ### fill in the blanks to make this list comprehension work
    Purple_YCoordinate = [agent.y for agent in agents if agent.group=="Purple"] # model this after the above list comprehension
    Gold_XCoordinate = [agent.x for agent in agents if agent.group=="Gold"]
    Gold_YCoordinate = [agent.y for agent in agents if agent.group=="Gold"]
...
class PurpleDiversitySeekers(AgentDiversity):
    def __init__(self, xlocation, ylocation, group="Purple", status="unhappy"):
        super().__init__(xlocation, ylocation, group, status)
...
class GoldDiversitySeekers(AgentDiversity):
    def __init__(self, xlocation, ylocation, group="Gold", status="unhappy"):
        super().__init__(xlocation, ylocation, group, status)
...
group_affinity_threshold = .51
testlist = [PurpleAgents(random.randint(0,100), random.randint(0,100))for i in range(300)] + [GoldAgents(random.randint(0,100), random.randint(0,100)) for i in range(300)] + [PurpleDiversitySeekers(random.randint(0,100), random.randint(0,100)) for i in range(300)] + [GoldDiversitySeekers(random.randint(0,100), random.randint(0,100)) for i in range(300)]  ### create a testlist that has 200 PurpleAgents and 200 GoldAgents
map_colorful_agents(testlist)
for x in range(15):
    for agent in (testlist):
        agent.check_neighbors(testlist)  # does this need any arguments?
